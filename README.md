...
- `content/blur.js` applies the blur (via inline styles, so it also works on elements rendered inside a shadow root) and handles click-to-reveal. The blur is applied in place, but the "click to reveal" label is a floating element positioned over the card via `getBoundingClientRect()` and kept in sync with a `requestAnimationFrame` loop — this avoids inserting any wrapper element into the page, which would otherwise break grid/flex layouts that size their children by tag or position
- `content.js` also detects in-app navigation (YouTube/X/Reddit all swap routes via client-side JS instead of a full page load) by watching `location.href`, and clears all blur/overlay state on a route change via `content/blur.js`'s `revealAll()` — otherwise a blurred element from the previous view can end up stuck on screen if the site reuses that DOM node for something else
- `content/observer.js` triggers a re-scan when new content loads — a debounced light-DOM `MutationObserver` plus a visibility-gated interval fallback for content inserted purely inside shadow roots. It deliberately avoids attaching a new observer per shadow root, which on a page as deeply shadow-nested as YouTube's can create thousands of observers and freeze the tab
